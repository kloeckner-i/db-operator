...
func (e *secretEventHandler) Update(evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
  ...
  case *corev1.Secret:
    ...
    if numDatabases > 1 {
      ...
      logrus.Error("Secret Update Event error! Multiple Database resources related to the same Secret: secret=", secretNew.Namespace,"/", secretNew.Name, ", dbNames=", dbNames)
      return
    }
